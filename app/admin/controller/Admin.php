...
class Admin extends Common
{
	...
	/**
	 * 获取菜单
	 * @param $role_id 角色id 1为超级管理员
	 */
	public function getMenus($role_id){
		$authRule = Loader::model('AuthRule');
		if($role_id==1)
			$levelData=$authRule->getLevelData();
		else{
			$authAccess = Loader::model('AuthAccess');
			$authAccessIds=$authAccess->getIdsByRoleid($role_id);
			$levelData=$authRule->getLevelData($authAccessIds);
		}
		$this->assign('menus',$levelData);
	}
	...
	public function checkRule($uid, $rule_val)
	{
		...
		$authAccess = Loader::model('AuthAccess');
		if(in_array(strtolower($rule_val), $authAccess->getRuleVals($uid))){
			return true;
		}
		return false;
	}
